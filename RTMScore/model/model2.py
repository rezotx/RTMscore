...
class MultiHeadAttentionLayer(nn.Module):
	"""Compute attention scores with a PyG graph's node and edge (geometric) features."""
	...
	def propagate_attention(self, edge_index, num_nodes, Q_h, K_h, V_h, proj_e):
		src, dst = edge_index
		# Compute attention scores: score = K_h[src] * Q_h[dst]
		...
		score = th.exp((score.sum(-1, keepdim=True)).clamp(-5.0, 5.0))

		# Compute weighted values: scatter sum of V_h[src] * score over dst
		V_h_src = V_h[src] * score  # [E, heads, feat_dim]
		wV = th.zeros(num_nodes, V_h.shape[1], V_h.shape[2],
					  device=edge_index.device, dtype=V_h.dtype)
		dst_expanded = dst.unsqueeze(-1).unsqueeze(-1).expand_as(V_h_src)
		wV.scatter_add_(0, dst_expanded, V_h_src)

		# Compute normalization: scatter sum of score over dst
		z = th.zeros(num_nodes, score.shape[1], score.shape[2],
					 device=edge_index.device, dtype=score.dtype)
		dst_expanded_score = dst.unsqueeze(-1).unsqueeze(-1).expand_as(score)
		z.scatter_add_(0, dst_expanded_score, score)

		return wV, z, e_out

	def forward(self, edge_index, node_feats, edge_feats, num_nodes):
		e_out = None
		...
		wV, z, e_out = self.propagate_attention(edge_index, num_nodes, Q_h, K_h, V_h, proj_e)
		# Compute final node and edge representations after multi-head attention
		h_out = wV / (z + th.full_like(z, 1e-6))  # Add eps to all
...
class GraphTransformerModule(nn.Module):
	...
	def run_gt_layer(self, edge_index, node_feats, edge_feats, num_nodes):
		...
		node_attn_out, edge_attn_out = self.mha_module(edge_index, node_feats, edge_feats, num_nodes)
	...
	def forward(self, edge_index, node_feats, edge_feats, num_nodes):
		...
		node_feats, edge_feats = self.run_gt_layer(edge_index, node_feats, edge_feats, num_nodes)
		return node_feats, edge_feats
...
class FinalGraphTransformerModule(nn.Module):
	...
	def run_gt_layer(self, edge_index, node_feats, edge_feats, num_nodes):
		...
		# Get multi-head attention output using provided node and edge representations
		node_attn_out, _ = self.mha_module(edge_index, node_feats, edge_feats, num_nodes)
		node_feats = node_attn_out.view(-1, self.num_output_feats)
		node_feats = F.dropout(node_feats, self.dropout_rate, training=self.training)
		node_feats = self.O_node_feats(node_feats)
	...
	def forward(self, edge_index, node_feats, edge_feats, num_nodes):
		...
		node_feats = self.run_gt_layer(edge_index, node_feats, edge_feats, num_nodes)
		return node_feats


class GraphTransformer(nn.Module):
	"""A graph transformer module.
	"""
	def __init__(
			self,
			in_channels,
	...
	def forward(self, g, node_feats, edge_feats):
		"""Forward pass. g is a PyG Data/Batch object."""
		...
		for gt_layer in self.gt_block[:-1]:
			node_feats, edge_feats = gt_layer(edge_index, node_feats, edge_feats, num_nodes)

		# Apply final layer to update node representations by merging current node and edge representations
		node_feats = self.gt_block[-1](edge_index, node_feats, edge_feats, num_nodes)
		return node_feats


def to_dense_batch_pyg(bg, feats, fill_value=0):
	"""Convert batched graph node features to dense batch format.

	Wrapper around torch_geometric.utils.to_dense_batch for compatibility.
	"""
	return to_dense_batch(feats, bg.batch, fill_value=fill_value)
...
class RTMScore(nn.Module):
	...
	def forward(self, bgp, bgl):
		h_l = self.lig_model(bgl, bgl.atom.float(), bgl.bond.float())
		h_p = self.prot_model(bgp, bgp.feats.float(), bgp.edge_feats.float())

		h_l_x, l_mask = to_dense_batch_pyg(bgl, h_l, fill_value=0)
		h_p_x, p_mask = to_dense_batch_pyg(bgp, h_p, fill_value=0)

		h_l_pos, _ =  to_dense_batch_pyg(bgl, bgl.pos, fill_value=0)
		h_p_pos, _ =  to_dense_batch_pyg(bgp, bgp.pos, fill_value=0)
		...
		bond_types = self.bond_types(th.cat([h_l[bgl.edge_index[0]],h_l[bgl.edge_index[1]]], axis=1))
