	...
	def __init__(self,
		...
		if isinstance(ligs,np.ndarray) or isinstance(ligs,tuple) or isinstance(ligs,list):
			if isinstance(ligs[0], Data):
				...
				raise ValueError('the variable "ligs" should be a set of (or a file to store) PyG Data objects.')
		else:
			try:
				self.graphsl = th.load(ligs, weights_only=False)
			except:
				raise ValueError('the variable "ligs" should be a set of (or a file to store) PyG Data objects.')
		...
		if isinstance(prots,np.ndarray) or isinstance(prots,tuple) or isinstance(prots,list):
			if isinstance(prots[0], Data):
				...
				raise ValueError('the variable "prots" should be a set of (or a file to store) PyG Data objects.')
		else:
			try:
				self.graphsp = th.load(prots, weights_only=False)
			except:
				raise ValueError('the variable "prots" should be a set of (or a file to store) PyG Data objects.')
		...
		if isinstance(ligs,np.ndarray) or isinstance(ligs,list):
			...
			if ligs.endswith(".mol2"):
				...
				try:
					...
					raise ValueError('Only the ligands with .sdf or .mol2 or a file to generate PyG Data objects will be supported')
